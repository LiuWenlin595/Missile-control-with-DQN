...
class DeepQNetwork:
    def __init__(self, n_actions, n_features, n_input, seed,
                 learning_rate=0.01,
                 reward_decay=0.9,
                 e_greedy_max=1,
        ...
        # 探索模式:若e_greedy_increment不为None,初始epsilon = 0;否则epsilon = epsilon_max
        ...
        if self.prioritized:
            ...
            # 行数:memory_size 列数:[s, a, r, done, s_]维数,状态量维数*2+动作维数+奖励值维数+done
            self.memory = np.zeros((self.memory_size, n_features * 2 + 3))

        # 构建目标网络,估计网络[target_net, evaluate_net]
        self._build_net()
    ...
    def _build_net(self):
        ...
        self.input_ = tf.placeholder(tf.float32, [None, self.n_input], name='input_')
        ...
        with tf.variable_scope('eval_net'):
            e1 = tf.layers.dense(self.input, 128, tf.nn.leaky_relu, kernel_initializer=w_initializer,
            ...
            e3 = tf.layers.dense(e2, 64, tf.nn.leaky_relu, kernel_initializer=w_initializer,
        ...
        with tf.variable_scope('target_net'):
            t1 = tf.layers.dense(self.input_, 128, tf.nn.leaky_relu, kernel_initializer=w_initializer,
            ...
            t3 = tf.layers.dense(t2, 64, tf.nn.leaky_relu, kernel_initializer=w_initializer,
    ...
    def learn(self):
        ...
        if self.prioritized:
            _, abs_errors, cost = self.sess.run([self._train_op, self.abs_errors, self.loss],
                     feed_dict={self.input: batch_memory[:, :self.n_input],
                                self.a: batch_memory[:, self.n_features],
                                self.r: batch_memory[:, self.n_features + 1],
                                self.done: batch_memory[:, self.n_features + 2],
                                self.input_: batch_memory[:, (self.n_features+3):],
            ...
            _, cost, tmp1, tmp2 = self.sess.run(
                ...
                feed_dict={
                    self.input: batch_memory[:, :self.n_input],
                    self.a: batch_memory[:, self.n_features],
                    self.r: batch_memory[:, self.n_features + 1],
                    self.done: batch_memory[:, self.n_features + 2],
                    self.input_: batch_memory[:, (self.n_features+3):],
    ...
    def plot_cost(self):
        ...
        plt.plot(np.arange(len(self.cost_his[500:])), self.cost_his[500:])
        plt.ylabel('Cost')
